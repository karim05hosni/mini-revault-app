...
export const DepositForm: React.FC<DepositFormProps> = ({ wallets, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(`Successfully deposited ${amount} ${selectedWallet?.currency}`);
      setAmount('');
      onSuccess();
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
      <h3 className="text-lg font-semibold text-slate-900 mb-5">Deposit Funds</h3>
