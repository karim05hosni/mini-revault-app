...
export const TransferForm: React.FC<TransferFormProps> = ({ wallets, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(`Successfully sent ${amount} ${selectedWallet?.currency} to ${recipientEmail}`);
      setRecipientEmail('');
      setAmount('');
      onSuccess();
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
      <h3 className="text-lg font-semibold text-slate-900 mb-5">Send Money</h3>
